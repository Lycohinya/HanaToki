...
/**
 * 把 instance/world/folia/config 幾個模組串起來的執行期核心,HanaTokiPlugin 只做 Bukkit
 * lifecycle 接線,狀態機邏輯都在這裡(方便 Phase 2+ 擴充,不塞進 plugin 主類別)。
 */
class HanaTokiCore(private val plugin: Plugin) : PresenceBridge {
    ...
    fun shutdownAll() {
        ...
        for (e in ended) handleSessionEnded(e.slotId, e.dungeonId, e.reason)
    }
